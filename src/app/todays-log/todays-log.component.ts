...
export class TodaysLogComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    // Create chart for each div
    this.createChart("todaysLogChartDiv1");
    this.createChart("todaysLogChartDiv2");
    this.createChart("todaysLogChartDiv3");
    this.createChart("todaysLogChartDiv4");
  }

  private createChart(divId: string): void {
    if (this.root) {
      this.root.dispose();  // Dispose of the existing root if it exists
    }
